...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        if self.use_flash_attention:
            ...
            weight_matrix = self.matmul(q, k) / (q_dim ** 0.5)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        # Reshape (B, T, D) → (B*T, D) for prenorm and projection layers
        ...
        # Reshape (B*T, H*d) → (B, H, T, d) for multi-head attention
        ...
        attn, probs = self.attn(q_prime, k_prime, v_prime)
        self.probs = probs

        # Merge heads: (B, H, T, d) → (B, T, H*d)
        attn = ops.transpose(attn, axes=(1, 2))
...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        # FFN: x = x + Dropout(Linear2(Dropout(ReLU(Linear1(LayerNorm(x))))))
        x_2d = x.reshape((batch_size * seq_len, x_dim))
        mlp_out = self.dropout(self.linear2(self.dropout(self.relu(self.linear1(self.layer_norm(x_2d))))))
        x = x + mlp_out.reshape((batch_size, seq_len, x_dim))
...
class Transformer(Module):
    ...
    def forward(
        ...
        pos_ids = Tensor(np.arange(seq_len, dtype="int32"), device=self.device)
        pos_ids = pos_ids.reshape((seq_len, 1)).broadcast_to((seq_len, batch_size))
        positional_embedding = ops.transpose(self.pos_embedding(pos_ids), axes=(1, 0))
