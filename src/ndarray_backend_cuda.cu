...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
   ...
   * See GPU linear-algebra references for cooperative fetching, shared-memory tiling, and
   * register blocking. This entry point sets up the kernel launch; heavy work lives in
   * MatmulKernel. Unlike the CPU tiled helper, this path must handle arbitrary matrix sizes.
   * 
   *
   * Args:
